...
- `index.md` must follow this structure — a `# Spec Files` header with Structure, Guidelines, and ToC sections, followed by grouped links to all spec files. Example:
...
- `index.md` (this file) — links to all spec files with one-line descriptions
- Per-module files — purpose, interfaces, behavior, edge cases, relevant configuration
- Specs exceeding ~300 lines should be split into sub-specs grouped in one directory

## Guidelines

- **High-level only**: describe *what* and *why*, not *how*. Types and interfaces to define contracts — never implementation code.
...
- **Size limit**: ~300 lines per file
- **Ratio**: ~1k lines of spec per 10k lines of code

## Specs

### <Group Name>

- [module-a.md](module-a.md) — one-line description
- [module-b.md](module-b.md) — one-line description

### <Group Name>

- [module-c.md](module-c.md) — one-line description
```

- Group spec links in the ToC by domain area (e.g. "Architecture & Infrastructure", "Domain Modules", "Cross-Cutting Systems")
...
This project maintains living specifications in `docs/specs/`.

**Before starting any task, read `<specs-dir>/index.md`** to understand the project structure, spec guidelines, and find relevant specs.

Ensure changes align with relevant specs. If specs need updating, use the brainstorming skill to evolve them first.
```
